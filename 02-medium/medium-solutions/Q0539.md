...
public class Solution {
    public int FindMinDifference(IList<string> timePoints) {
        ...
        for (int i = 0; i < timePoints.Count; i++) {
            minutes.Add(TimeToMins(timePoints[i]));
        }
        
        minutes.Sort();

        int min_diff = 1440, n = minutes.Count;

        for (int i = 0; i < n-1; i++) {
            int t1 = minutes[i];
            int t2 = minutes[i+1];
            if (t1 == t2) return 0;
            min_diff = Math.Min(t2 - t1, min_diff);
        }

        min_diff = Math.Min(min_diff, 1440 + minutes[0] - minutes[n-1]);
